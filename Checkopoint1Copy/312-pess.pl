...
process2([load]):- write("Enter a file name in double quotes."), nl,
                  ...
                  string_to_atom(S1, G),
                  load_rules(G),
                  read_sentence(G),  % read sentence after the rules load 
                 % write(G),  % write out the second input after loading the rules.
                  process2(G),  % %
                  !
                  . 
...
process2([solve]):- solve, read_sentence(S), process2(S).   %% this calls the solve function!!

process2([quit]):- write("exiting the shell!!"), abort.
...
solve :-
        ...
        read_sentence(S), 
        process2(S).  
        % after finding the answer it should go back to the shell...
solve :-
