...
%read_sent_helper([Word|Words]) :-            % Read quoted words.
%        peek_char(Ch), Ch = ''', !,           % read single quoted words
%        read_word_to(ChWord), 
%        atom_chars(Word, ChWord), 
%        read_sent_helper(Words).
...
adjp_star(APTerms) -->
        adjp(FstAPTerms),
        adjp_star(RestAPTerms),  % null and we have our adjectives late...
        { append(FstAPTerms, RestAPTerms, APTerms) }.
adjp_star([]) --> []. 

% An "adjective phrase", which may include adverbs.
adjp(APTerms) -->
        adv_star(AVTerms),   % null... yess...
        adj(AdjTerms),        % this should find late... 
...
% ---- > alvins function here... 
% if there are two empty words... 
% we are going to call this with build_facts(Outside,Inside,Fact). 
% check if this works in 312.pess.pl... !!
%build_facts([],[], _). 
% have one each for either noun, verb, adjective, adverb...

% arguments for inside and the outside for build facts... 
%attr(is_a,thing,[])],attr(is_a,noun,[])
% how would i access that?  attr
% we need a predicate that extracts the element within the fact.
% maybe we can use the member predicate??
%build_facts(Outside,Inside, Fact):- member(noun, Outside),   
%                                    build_facts([], [], [noun(Inside)]).
                                    

%build_facts(Outside,Inside, Fact):- Outside = "verb", 
%                                    build_facts([], [], [verb(Inside)]).
